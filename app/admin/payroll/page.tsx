...
export default function PayrollPage() {
  ...
  useEffect(() => { if (employees.length > 0) fetchPayrollForMonth() }, [selectedYear, selectedMonth, employees])

  async function fetchEmployees() {
    const { data } = await supabase.from('employees').select('id, name').is('resign_date', null).order('name')
    setEmployees(data || [])
  }
  ...
  async function fetchPayrollForMonth() {
    ...
    const newRows: PayrollRow[] = employees.map((emp) => {
      ...
      return ex ? { ...ex } : { employee_id: emp.id, employee_name: emp.name, year: selectedYear, month: selectedMonth, base_salary: 0, bonus: 0, deduction: 0, net_salary: 0, note: '', status: 'draft' as const }
    })
    setRows(newRows)
  ...
  function handleRowChange(idx: number, field: 'base_salary' | 'bonus' | 'deduction' | 'note', value: string) {
    setRows((prev) => {
      const next = [...prev]
      const row = { ...next[idx] }
      if (field === 'note') { row.note = value }
      else { row[field] = parseInt(value.replace(/,/g, ''), 10) || 0 }
  ...
  async function handleSaveDraft() {
    ...
    for (const row of rows) {
      const payload = { employee_id: row.employee_id, employee_name: row.employee_name, year: row.year, month: row.month, base_salary: row.base_salary, bonus: row.bonus, deduction: row.deduction, net_salary: row.net_salary, note: row.note || null, status: 'draft' }
  ...
  async function handleFinalize() {
    ...
    const finalPayloads = rows.map((row) => ({ employee_id: row.employee_id, employee_name: row.employee_name, year: row.year, month: row.month, base_salary: row.base_salary, bonus: row.bonus, deduction: row.deduction, net_salary: row.net_salary, note: row.note || null, status: 'final' }))
        ...
        <div className="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden">
          ...
          {tab === 'input' ? (
            <div>
              ...
              {rows.length > 0 && (
                <div className="px-6 py-3 bg-blue-50 border-b border-blue-100 flex gap-6 text-sm">
                  ...
                  <span className="text-blue-700">기본급 합계: <strong>{formatKRW(totalBase)}</strong></span>
                  <span className="text-blue-700">실지급 합계: <strong>{formatKRW(totalNet)}</strong></span>
                </div>
              )}
              {fetchLoading ? <div className="py-12 text-center text-gray-400 text-sm">불러오는 중...</div>
              : rows.length === 0 ? <div className="py-12 text-center text-gray-400 text-sm">등록된 직원이 없습니다.</div>
              : (
                <>
                          ...
                          <tr key={row.employee_id} className="hover:bg-gray-50">
                            ...
                            {(['base_salary', 'bonus', 'deduction'] as const).map((field) => (
                              <td key={field} className="px-4 py-2">
                                <input type="text" value={row[field] === 0 ? '' : row[field].toLocaleString('ko-KR')}
                                  onChange={(e) => handleRowChange(idx, field, e.target.value)}
                                  disabled={row.status === 'final'} placeholder="0"
                            ...
                            <td className="px-4 py-2 font-semibold text-blue-700 text-right whitespace-nowrap">{formatKRW(row.net_salary)}</td>
                            <td className="px-4 py-2">
                              <input type="text" value={row.note} onChange={(e) => handleRowChange(idx, 'note', e.target.value)}
                                disabled={row.status === 'final'} placeholder="비고"
                  ...
                  {!isFinalized && (
                    <div className="px-6 py-4 border-t border-gray-100 flex gap-3 justify-end">
                      <button onClick={handleSaveDraft} disabled={loading} className="border border-gray-300 hover:bg-gray-50 text-gray-700 text-sm font-medium px-5 py-2 rounded-md transition-colors disabled:opacity-50">
                        {loading ? '저장 중...' : '💾 임시저장'}
                      </button>
                      <button onClick={handleFinalize} disabled={loading} className="bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium px-5 py-2 rounded-md transition-colors disabled:opacity-50">
                        ✓ 최종 확정
                      </button>
                    </div>
                  )}
            ...
            <div>
              ...
              {fetchLoading ? <div className="py-10 text-center text-gray-400 text-sm">불러오는 중...</div>
              : savedRows.length === 0 ? <div className="py-10 text-center text-gray-400 text-sm">급여 데이터가 없습니다.</div>
              : (
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead className="bg-gray-50 text-gray-500 text-xs">
