...
export default function LeaveStatusPage() {
  ...
  async function fetchLeaveData() {
    ...
    for (const req of allReqs) {
      ...
      if (req.status !== 'rejected') {
        byEmployee[key].used += Number(req.days_used)
      }
    }

    // leave_balance 테이블에서 부여일수 조회
    ...
    if (balances) {
      for (const b of balances) {
        const match = Object.values(byEmployee).find(
          (s) => s.employee_id === b.employee_id
        )
        if (match) {
          match.granted = b.total_granted
        }
      }
    }

    for (const s of Object.values(byEmployee)) {
      s.remaining = s.granted - s.used
    }
  ...
  async function handleReject(id: string) {
    ...
    const { error } = await supabase
      .from('leave_requests')
      .update({ status: 'rejected', note: reason || undefined })
      .eq('id', id)
    if (error) setError(error.message)
    else fetchLeaveData()
  }
        ...
        <div className="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden">
          ...
          {tab === 'summary' ? (
            loading ? (
              ...
              <div className="py-12 text-center text-gray-400 text-sm">{selectedYear}년 데이터가 없습니다.</div>
            ) : (
                    ...
                    {summaries.map((s) => {
                      ...
                      return (
                        <tr key={s.employee_name} className="hover:bg-gray-50">
                          ...
                          <td className={`px-4 py-3 text-center font-bold ${s.remaining <= 3 ? 'text-red-600' : 'text-green-600'}`}>
                            {s.remaining}일
                          </td>
                          <td className="px-4 py-3 text-center">
                            <div className="flex items-center gap-2 justify-center">
                              <div className="w-20 bg-gray-200 rounded-full h-1.5">
                                <div
                                  className={`h-1.5 rounded-full ${usageRate >= 80 ? 'bg-red-500' : usageRate >= 50 ? 'bg-orange-400' : 'bg-green-500'}`}
                          ...
                          <td className="px-4 py-3 text-xs text-gray-500">
                            {lastReq ? (
                              <>
                                {lastReq.leave_date} ·{' '}
                                <span className={`px-1.5 py-0.5 rounded text-xs ${STATUS_LABEL[lastReq.status].className}`}>
                                  {STATUS_LABEL[lastReq.status].label}
                                </span>
                              </>
                            ) : '-'}
                          </td>
            ...
            <div>
              ...
              {loading ? (
                ...
                <div className="py-10 text-center text-gray-400 text-sm">해당하는 신청이 없습니다.</div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead className="bg-gray-50 text-gray-500 text-xs uppercase">
