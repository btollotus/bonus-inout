...
export default function LeavePage() {
  ...
  function calcLegalLeaveDays(hireDateStr: string | null | undefined, targetYear: number): number {
    ...
    if (yearsWorked < 1) {
      // 1년 미만: 입사월부터 연말까지 개근 월수 (최대 11일)
      const hireYear = hire.getFullYear()
      const hireMonth = hire.getMonth() // 0-based
      let months = 0
      if (hireYear < targetYear) {
        months = 11 // 전년도 입사: 1~11월 = 최대 11일
      } else {
        months = Math.min(11 - hireMonth, 11)
      }
  ...
  async function fetchAllBalances() {
    ...
    for (const e of (empList || [])) {
      ...
      if (!bal) {
        ...
        balMap[e.id] = { ...bal, total_days: legalDays, total_granted: legalDays, remaining_days: Math.max(0, legalDays - usedNow) }
      }
    }
  ...
  // 수동 조정 저장
  async function saveManualOverride(empId: string) {
    const emp = allBalances.find(b => b.employee_id === empId)
    if (!emp) return
    ...
    if (existing) {
      ...
      const { error: insErr } = await supabase.from('leave_balance').insert({
        ...
        remaining_days: Math.max(0, total - used),
        manual_override: true,
        override_reason: editDraft.override_reason || null,
      })
  ...
  async function fetchData() {
    ...
    if (!emp) {
      ...
      setBalance((prev) => prev ? { ...prev, used_days: used, remaining_days: Math.max(0, prev.total_days - used) } : null)
    }
