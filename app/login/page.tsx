...
const cardClass = "bg-white rounded-2xl shadow-md border border-gray-200 p-8 w-full max-w-md"
const wrapClass = "flex items-center justify-center px-4 py-12"
...
export default function LoginPage() {
      ...
      supabase.auth.setSession({ access_token: accessToken, refresh_token: refreshToken })
        .then(({ data, error }) => {
          if (error || !data.session) {
            setError('링크가 만료되었습니다.'); setMode('login')
          } else {
  ...
  function ErrMsg() { return error ? <div className="bg-red-50 border border-red-200 text-red-700 text-sm px-4 py-2.5 rounded-md mb-4">{error}</div> : null }
  function OkMsg() { return success ? <div className="bg-green-50 border border-green-200 text-green-700 text-sm px-4 py-2.5 rounded-md mb-4">{success}</div> : null }
  function Footer() { return <p className="text-xs text-center text-gray-400 mt-6">(주)보누스메이트 ERP</p> }
  function BackBtn() { return <button onClick={goBack} className="mt-4 w-full text-center text-sm text-gray-500 hover:text-gray-700">← 로그인으로 돌아가기</button> }

  if (mode === 'checking') return <div className={wrapClass}><div className="text-gray-500 text-sm">확인 중...</div></div>
    ...
    <div className={wrapClass}>
      <div className={cardClass}>
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          <div>
            ...
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)}
              placeholder="이메일 입력" required autoComplete="email" className={inputClass} />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">비밀번호</label>
            <input type="password" value={password} onChange={(e) => setPassword(e.target.value)}
              placeholder="비밀번호 입력" required autoComplete="current-password" className={inputClass} />
          </div>
        ...
        <div className="mt-5 flex justify-center gap-4 text-sm">
          <button onClick={() => { setMode('forgot-password'); setError('') }} className="text-blue-600 hover:underline">비밀번호 찾기</button>
          <span className="text-gray-300">|</span>
          <button onClick={() => { setMode('find-email'); setError('') }} className="text-blue-600 hover:underline">이메일 찾기</button>
        </div>
        <Footer />
      </div>
    </div>
      ...
      <div className={cardClass}>
        ...
        <form onSubmit={handleForgotPassword} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">이메일</label>
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)}
              placeholder="가입한 이메일 입력" required autoComplete="email" className={inputClass} />
          </div>
          <button type="submit" disabled={loading}
        ...
        <form onSubmit={handleFindEmail} className="space-y-4">
          <div>
            ...
            <input type="text" value={findName} onChange={(e) => setFindName(e.target.value)}
              placeholder="홍길동" required autoComplete="name" className={inputClass} />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">생년월일 (주민번호 앞 6자리) *</label>
            <input type="text" inputMode="numeric"
              value={findBirthdate}
              onChange={(e) => setFindBirthdate(e.target.value.replace(/[^0-9]/g, '').slice(0, 6))}
              placeholder="901225" maxLength={6} required
              className={inputClass + " font-mono tracking-widest"} />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">휴대폰 번호 *</label>
            <input type="tel" value={findMobile}
              onChange={(e) => setFindMobile(formatPhone(e.target.value))}
              placeholder="010-0000-0000" required autoComplete="tel" className={inputClass} />
          </div>
          <button type="submit" disabled={loading}
            className="w-full bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white font-semibold py-2.5 rounded-lg transition-colors">
            {loading ? '조회 중...' : '이메일 찾기'}
    ...
    <div className={wrapClass}>
      <div className={cardClass}>
        ...
        <div className="text-center">
          ...
          <button onClick={() => { setMode('login'); setEmail(''); setError('') }}
      ...
      <div className={cardClass}>
        ...
        <form onSubmit={handleSetPassword} className="space-y-4">
          <div>
            ...
            <input type="password" value={password} onChange={(e) => setPassword(e.target.value)}
              placeholder="6자 이상 입력" required autoComplete="new-password" className={inputClass} />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">비밀번호 확인</label>
            <input type="password" value={confirm} onChange={(e) => setConfirm(e.target.value)}
              placeholder="동일한 비밀번호 재입력" required autoComplete="new-password" className={inputClass} />
          </div>
          <button type="submit" disabled={loading}
            className="w-full bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white font-semibold py-2.5 rounded-lg transition-colors">
