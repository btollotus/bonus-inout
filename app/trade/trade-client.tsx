...
type Line = { food_type: string; name: string; weight_g: number | string; qty: number; unit: number | string; total_incl_vat: number | string; is_sample?: boolean; stock_out_lots?: { lot_id: string; qty: string }[]; logo_spec?: string; _woItemId?: string; school_name?: string; gift_qty?: number | string };
...
<div className="md:col-span-3"><div className="mb-1 text-xs text-slate-600">메모</div><input className={inp} lang="ko" value={ledgerMemo} onChange={(e) => setLedgerMemo(e.target.value)} /></div>
            ...
            <div className={`${card} p-4`}>
              ...
              {pendingPrepaidOrders.length > 0 ? (
                <div className="mb-4">
                  <div className="mb-2 flex items-center justify-between">
                    <div className="text-sm font-semibold text-red-600">[자동입력]세금계산서 선발행·미입금 주문 {pendingPrepaidOrders.length}건</div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="text-sm font-semibold text-red-600">[수동입력]세금계산서 선발행·미입금 주문</div>
  ...
  <>
    ...
    <button className={`${miniBtn} col-span-2`} onClick={async () => {
      const { data } = await supabase.from("work_orders").select("*,work_order_items(id,work_order_id,delivery_date,sub_items,order_qty,actual_qty,unit_weight,total_weight,expiry_date,order_id,barcode_no,note,images,logo_spec,school_name)").eq("linked_order_id", x.rawId).limit(1).maybeSingle();
