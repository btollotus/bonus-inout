...
export function calculateQuote(productKey, input) {
  ...
  if (!PRODUCTS[productKey]) throw new Error(`알 수 없는 제품: ${productKey}`)
  if (p.manual) throw new Error(`${p.label}은 수동 계산 제품입니다.`)
    ...
    if (productKey === '롤리팝레이즈-별도' && width) {
      const perSheet = Math.floor(430 / (width + 5))
      if (perSheet > 0) p = { ...p, sheetFixed: perSheet }
    }
  ...
  // - 기성 성형틀 (useStockMold=true): 성형틀 제작비 없음, 대신 V_stock = V×1.2 적용
  const moldActual  = (isNew && !reuseExistingMold && !useStockMold) ? K : 0
 ...
 * @param isNew   신규 여부 — true이면 인쇄제판 95,000원 발생, false이면 면제
 *
 * 요금 구조:
 *   인쇄제판 = 95,000원 (신규) / 0원 (재주문)
 *   전사지   = sheets × 3,000원
 *   공급가   = 인쇄제판 + 전사지
...
export function calculateSheetQuote(input) {
  ...
  const plateCost   = isNew ? 95000 : 0              // 인쇄제판
